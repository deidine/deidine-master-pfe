...
export default function BadgeElement({
  ...
  return (
    <div className="flex flex-col ">
      <div className=" p-2 w-full font-bold text-lg h-auto overflow-hidden">{element.label}</div>
      <div className="flex flex-col w-auto items-start space-y-2">
        <div className="flex flex-col gap-2 w-auto items-start  " onClick={onClick}>
          ...
          <Button size="small" className="bg-blue-500"> {element.placeholder}</Button>
            ...
            {element.options && (
              <Button  className="bg-green-500" size="small">options
              <div className={`rounded-full w-6 h-6 ${ element.options.length != 2 ? "bg-yellow-700 ": "bg-red-700"}`}>
