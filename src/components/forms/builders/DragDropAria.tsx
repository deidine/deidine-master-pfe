  ...
  import { useTranslation } from 'react-i18next'; // Import useTrans 
  ...
  const { t } = useTranslation(); 
    ...
    const handleOnDragEnd = (result: any) => {
      ...
      if (!result.destination) return; // Check if destination exists
          ...
          {(provided) => (
            <div ref={provided.innerRef} {...provided.droppableProps}>
              {elements.map((element, index) => (
                <Draggable
                  key={index}
                  draggableId={"" + index} // Ensure unique ID for each draggable item
                  index={index}
                >
                  {(provided, snapshot) => (
                    <div
